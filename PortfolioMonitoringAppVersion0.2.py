...
SPY = adj_close.ix[:, 'SPY']
SPYfig = plt.figure()
SPYfig = SPYfig.add_subplot(1,1,1)
SPYfig.plot(NZX50.index, NZX50, label='SPY Index')
SPYfig.set_xlabel('Date')
SPYfig.set_ylabel('Adjusted closing price ($USD)')
SPYfig.legend()
...
class PortfolioMonitorApp(tk.Tk): # PMA class inheriting all attributes from
    ...
    # contained in this method will start when the PMA class is called.
        ...
        self.frames = {}
        for F in (AgreementPage, HomePage, NZX50Index, SPYIndex, TestPage):
...
class HomePage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button3 = ttk.Button(self, text="Exit",
                             command=closeProgram)
        button3.pack()
        button4 = ttk.Button(self, text="SPY Index",
                             command=lambda: controller.show_frame(SPYIndex))
        button4.pack()
...
class SPYIndex(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        label = tk.Label(self, text="SPY Index", font=LARGE_FONT)
        label.pack(pady=10,padx=10)
        button1 = ttk.Button(self, text="Back to Home",
                            command=lambda: controller.show_frame(HomePage))
        button1.pack()
                
        canvas = FigureCanvasTkAgg(SPYfig, self)
        canvas.show()
        canvas.get_tk_widget().pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True)
